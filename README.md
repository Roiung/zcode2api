...
将 ZCode (zcode.z.ai) Coding Plan 额度转为标准 Anthropic Messages API，并额外暴露 OpenAI Chat Completions 兼容接口，支持多账号轮询、
...
- 对话端点：`http://localhost:3000/v1/messages`（Anthropic Messages 协议）
- OpenAI 兼容端点：`http://localhost:3000/v1/chat/completions`
- OpenAI Responses 兼容端点：`http://localhost:3000/v1/responses`
...
- **网关鉴权（可选）**：在「设置」配置「网关 API Key」后，`/v1/messages` / `/v1/chat/completions` 须携带
  `Authorization: Bearer <key>` 或 `x-api-key: <key>`；留空则不校验。

## OpenAI 兼容接口

- 新增 `POST /v1/chat/completions`，用于兼容 OpenAI 风格客户端。
- 新增 `POST /v1/responses`，用于兼容新版 OpenAI Responses API 客户端。
- 内部会将 OpenAI 请求转换到 Anthropic `/v1/messages` 主链路，因此仍复用现有多账号轮询、验证码与换号逻辑。
- 已支持文本对话、SSE 流式输出，以及 `tools` / `tool_choice` / `tool_calls` 的基础兼容映射。
- `responses` 端点当前已支持 `input`、函数工具调用链路与基础流式事件映射。
- 当前兼容重点仍是函数工具调用；更复杂的多模态字段与更细粒度事件语义，后续如有需要可继续细化。
